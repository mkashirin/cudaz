...
pub fn cudaText(
    cuda_text: [:0]const u8,
    options: ?Options,
    allocator: std.mem.Allocator,
) CompileError![:0]const u8 {
    var program: nvrtc.nvrtcProgram = undefined;
    try Error.fromNvrtcErrorCode(nvrtc.nvrtcCreateProgram(
        &program,
        cuda_text.ptr,
        null,
        0,
...
pub fn cudaProgram(
    ...
    Error.fromNvrtcErrorCode(nvrtc.nvrtcCompileProgram(
        prg,
        std.math.cast(c_int, cparams.len).?,
        cparams.ptr,
    )) catch |e| {
        switch (e) {
            error.NVRTC_ERROR_COMPILATION => {
                var log_size: usize = undefined;
                try Error.fromNvrtcErrorCode(
                    nvrtc.nvrtcGetProgramLogSize(prg, &log_size),
                );
                const log_msg = try allocator.allocSentinel(
                    u8,
                    log_size,
                    '0',
                );
                try Error.fromNvrtcErrorCode(
                    nvrtc.nvrtcGetProgramLog(prg, log_msg.ptr),
                );
                std.debug.print("{s}\n", .{log_msg});
