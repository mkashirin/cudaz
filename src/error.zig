            ...
            for (x.decls) |declaration| {
                if (std.mem.startsWith(u8, declaration.name, prefix)) {
                    errors[counter] = .{
                        ...
                        .value = @field(cimport, declaration.name),
                    };
                    counter += 1;
                }
            }
...
test "CUDA errors generation" {
    // ```
    // zig test error.zig -lcuda -I /usr/local/cuda-12.6/targets/x86_64-linux/include
    // ```
    const test_cuda_error_enums = ErrorsToEnum(u32, cuda);
    const test_some_struct = utils.EnumToError(test_cuda_error_enums);
    try std.testing.expect(
        test_some_struct.from_error_code(1) orelse unreachable ==
            error.CUDA_ERROR_INVALID_VALUE,
    );
}

test "NVRTC errors generation" {
    const test_cuda_error_enums = ErrorsToEnum(u32, nvrtc);
    const test_some_struct = utils.EnumToError(test_cuda_error_enums);
    try std.testing.expect(
        test_some_struct.from_error_code(1) orelse unreachable ==
            error.NVRTC_ERROR_OUT_OF_MEMORY,
    );
}

test "Curand errors generation" {
    const test_cuda_error_enums = ErrorsToEnum(u32, curand);
    const test_some_struct = utils.EnumToError(test_cuda_error_enums);
    try std.testing.expect(
        test_some_struct.from_error_code(100) orelse unreachable ==
            error.CURAND_STATUS_VERSION_MISMATCH,
    );
}
